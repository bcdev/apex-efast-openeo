...
@pytest.mark.openeo
@pytest.mark.manual
def test_distance_to_cloud_s2_pixels(
    s2_cube,
    connection,
...
@pytest.mark.openeo
@pytest.mark.manual
def test_distance_to_cloud_s2_metres(
    s2_cube,
    connection,
    persistent_output_dir,
    image_size_pixels,
...
@pytest.mark.openeo
@pytest.mark.manual
def test_download_input_cube_s2(
    s2_cube, connection, persistent_output_dir, file_extension
):
...
@pytest.mark.openeo
@pytest.mark.manual
def test_download_cloud_mask(
    s2_cube, connection, persistent_output_dir, file_extension, execute
):
    cloud_mask = compute_cloud_mask_s2(s2_cube.band(CLOUD_BAND_S2))
    execute(cloud_mask)(
        (persistent_output_dir / "cloud_mask_s2").with_suffix(file_extension)
    )
...
@pytest.mark.openeo
@pytest.mark.manual
def test_distance_to_cloud_s3_pixels(
    s3_cube,
    connection,
    persistent_output_dir,
    image_size_pixels,
...
@pytest.mark.openeo
@pytest.mark.manual
def test_distance_to_cloud_s3_degrees(
    s3_cube,
    connection,
    persistent_output_dir,
    image_size_pixels,
...
@pytest.mark.openeo
@pytest.mark.manual
def test_download_input_cube_s3(
    s3_cube, connection, persistent_output_dir, file_extension
):
...
@pytest.mark.manual
def test_download_cloud_mask(
    s3_cube, connection, persistent_output_dir, file_extension, execute
):
    cloud_mask = compute_cloud_mask_s3(s3_cube.band(CLOUD_BAND_S3))
    execute(cloud_mask)(
        (persistent_output_dir / "cloud_mask_s3").with_suffix(file_extension)
    )
