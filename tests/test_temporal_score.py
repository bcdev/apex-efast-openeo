...
@pytest.mark.openeo
@pytest.mark.manual
def test_temporal_score_aggregate(
    time_frame, s3_cube, persistent_output_dir, execute, file_extension
):
    ...
    execute(score)(
        (persistent_output_dir / "temporal_score_agg").with_suffix(file_extension)
    )
